...
namespace Common.MultiThread
{
    ...
    public sealed class DispatcherContainer : FrameworkElement
    {
        ...
        {
            await SetChildAsync(() => new T(), () => new T(), dispatcher);
        }

        public async Task SetChildAsync<T>(Func<T> @new, Func<T> @new2, [CanBeNull] Dispatcher dispatcher = null)
            where T : UIElement
        {
            //  if (_isUpdatingChild) return;
            ...
            //    await _targetSource.Dispatcher.InvokeAsync(_targetSource.Dispose);
            //    _targetSource = null;
            //    _child = null;
            //} 
            //Thread.Sleep(2);
            ...
            async Task SetChildAsync()
            {
                ...
                else
                {
                    await value.Dispatcher.InvokeAsync(() =>
                    {
                        ...
                        // Console.WriteLine("完成："  + "___ThreadId：" + Thread.CurrentThread.ManagedThreadId);
                    });
